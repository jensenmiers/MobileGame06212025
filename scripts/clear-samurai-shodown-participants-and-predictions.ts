/**
 * Script to clear all Samurai Shodown participants and their related predictions
 */

import { database } from '../src/lib/database';

async function clearSamuraiShodownParticipantsAndPredictions() {
  console.log('🗑️  Clearing all Samurai Shodown participants and predictions\n');
  console.log('=' .repeat(60));

  try {
    // First, get the Samurai Shodown tournament ID
    const { data: tournament, error: tournamentError } = await database
      .from('tournaments')
      .select('id, name')
      .eq('name', 'Samurai Shodown')
      .single();
    ...
    if (!tournament) {
      console.error('❌ Samurai Shodown tournament not found in database');
      return;
    }
...
async function main() {
  ...
  await clearSamuraiShodownParticipantsAndPredictions();
}
